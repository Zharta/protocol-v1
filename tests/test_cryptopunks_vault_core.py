...
def test_propose_owner_zero_address(cryptopunks_vault_core_contract, contract_owner):
    with ape.reverts("address it the zero address"):
        cryptopunks_vault_core_contract.proposeOwner(ape.ZERO_ADDRESS, {"from": contract_owner})
...
def test_claim_ownership(cryptopunks_vault_core_contract, contract_owner, borrower):
    ...
    assert cryptopunks_vault_core_contract.proposedOwner() == ape.ZERO_ADDRESS
...
def test_set_collateral_vault_address_peripheral_zero_address(cryptopunks_vault_core_contract, contract_owner):
    with ape.reverts("address is the zero addr"):
        cryptopunks_vault_core_contract.setCollateralVaultPeripheralAddress(ape.ZERO_ADDRESS, {"from": contract_owner})
...
def test_store_collateral_wrong_sender(cryptopunks_vault_core_contract, contract_owner):
    with ape.reverts("msg.sender is not authorised"):
        cryptopunks_vault_core_contract.storeCollateral(contract_owner, ape.ZERO_ADDRESS, 0, {"from": contract_owner})
...
def test_store_collateral_invalid_contract(cryptopunks_vault_core_contract, collateral_vault_peripheral_contract, erc721_contract, borrower, contract_owner):
    ...
    with ape.reverts("address not supported by vault"):
        cryptopunks_vault_core_contract.storeCollateral(contract_owner, ape.ZERO_ADDRESS, 0, {"from": collateral_vault_peripheral_contract})
...
def test_transfer_collateral_wrong_sender(cryptopunks_vault_core_contract, contract_owner):
    with ape.reverts("msg.sender is not authorised"):
        cryptopunks_vault_core_contract.transferCollateral(contract_owner, ape.ZERO_ADDRESS, 0, {"from": contract_owner})
