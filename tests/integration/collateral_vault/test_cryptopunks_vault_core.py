...
def test_store_collateral_wrong_sender(cryptopunks_vault_core_contract, contract_owner):
    with boa.reverts("msg.sender is not authorised"):
        cryptopunks_vault_core_contract.storeCollateral(contract_owner, ZERO_ADDRESS, 0, ZERO_ADDRESS, sender=contract_owner)
...
def test_store_collateral_invalid_contract(cryptopunks_vault_core_contract, collateral_vault_peripheral_contract, erc721_contract, borrower, contract_owner):
    ...
    with boa.reverts("address not supported by vault"):
        cryptopunks_vault_core_contract.storeCollateral(contract_owner, ZERO_ADDRESS, 0, ZERO_ADDRESS, sender=collateral_vault_peripheral_contract.address)
...
def test_store_collateral(
    ...
    borrower = cryptopunks_market_contract.punkIndexToAddress(0)
    cryptopunks_market_contract.offerPunkForSaleToAddress(0, 0, cryptopunks_vault_core_contract.address, sender=borrower)
    cryptopunks_vault_core_contract.setCollateralVaultPeripheralAddress(collateral_vault_peripheral_contract, sender=contract_owner)

    cryptopunks_vault_core_contract.storeCollateral(borrower, cryptopunks_market_contract, 0, ZERO_ADDRESS, sender=collateral_vault_peripheral_contract.address)
...
def test_transfer_collateral_wrong_sender(cryptopunks_vault_core_contract, contract_owner):
    with boa.reverts("msg.sender is not authorised"):
        cryptopunks_vault_core_contract.transferCollateral(contract_owner, ZERO_ADDRESS, 0, contract_owner, sender=contract_owner)
...
def test_transfer_collateral(
    ...
    cryptopunks_vault_core_contract.transferCollateral(borrower, cryptopunks_market_contract, 0, borrower, sender=collateral_vault_peripheral_contract.address)
