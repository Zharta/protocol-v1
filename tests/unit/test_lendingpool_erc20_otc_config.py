...
def test_deprecate_blockers(lendingpool_otc, lendingpool_otc_proxy, weth9_contract):
    ...
    weth = weth9_contract.deploy("ERC20", "ERC20", 18, 10**20, override_address=ERC20_ADDRESS)
    weth.deposit(sender=LENDER, value=10)
    weth.approve(lendingpool_otc_proxy.address, 10, sender=LENDER)
    ...
    with boa.reverts("pool is deprecated, withdraw"):
        lendingpool_otc_proxy.deposit(1, sender=LENDER)

    with boa.reverts("pool is deprecated"):
        lendingpool_otc_proxy.sendFunds(LENDER, 1, sender=LENDER)
