...
def test_load_contract_config(contracts_config, cryptopunk_collaterals):
...
def test_add_liquidation(
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract, 0))
    ...
    assert liquidation[0] == liquidation_id
...
def test_pay_loan_liquidations_grace_period(
    ...
    liquidation1 = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract.address, 0))
    liquidation2 = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract.address, 1))
...
def test_buy_nft_lender_period_not_lender(
    ...
    lending_pool_peripheral_contract.depositWeth(LOAN_AMOUNT * 2)
    lending_pool_peripheral_contract.sendFundsWeth(contract_owner, LOAN_AMOUNT, sender=loans_peripheral_contract.address)
...
def test_buy_nft_lender_period(
    ...
    liquidation_id = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract.address, 0))[0]
    loan = loans_core_contract.getLoan(borrower, loan_id)
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract.address, 0))
...
def _create_liquidation(
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract.address, 0))
    loan = checksummed(loans_core_contract.getLoan(borrower, loan_id))
...
def test_admin_liquidation(
    ...
    liquidations_peripheral_contract.adminLiquidation(
        ...
        LOAN_AMOUNT*1//10,
        investedAmount,
        bytes.fromhex(liquidation_id[2:]),
        liquidation[14],
        collateral_address,
        token_id,
        sender=contract_owner
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(erc721_contract.address, 0))
    assert int(liquidation[0], base=16) == 0
    assert liquidation[1] == ZERO_ADDRESS
    assert liquidation[3] == 0
    assert liquidation[11] == ZERO_ADDRESS
    assert liquidation[14] == ZERO_ADDRESS
...
def test_cryptopunks_nftx_buy(
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(cryptopunks_market_contract, 0))
    loan = loans_core_contract.getLoan(borrower, loan_id)
...
def test_hashmasks_nftx_buy(
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(hashmasks_contract.address, token_id))
    loan = loans_core_contract.getLoan(borrower, loan_id)
...
def test_wpunks_nftx_buy(
    ...
    liquidation = checksummed(liquidations_peripheral_contract.getLiquidation(wpunks_contract, token_id))
    loan = loans_core_contract.getLoan(borrower, loan_id)
