...
def get_event(node: dict):
    ...
    attrs = [(nested_get(a, "target", "id"),
              nested_get(a, "annotation", "id") or nested_get(a, "annotation", "args", default=[])[0]["id"],
...
def get_function(node: dict):
    ...
    return_node = node.get("returns") or {}
    return_type = return_node.get("id")
...
def get_public_var(node: dict):
    ...
    if nested_get(node, "annotation", "ast_type") == "Subscript":
        ...
        if nested_get(annotation, "value", "id") == "HashMap":
            elements = nested_get(annotation, "slice", "value", "elements")
            args = [get_arg_type(e) for e in elements]
            return_type = args[1]
            args = [("arg0", args[0])]
